...
class CML:
    def __init__(self, mu, sigma, density, dataIn, distIn, szIn, ploidy, approx=True):
        self.k = ploidy
        self.mu = mu
    ...
    def apx_bootstrap_helper(self,stat,samples,dClass,sz,sigma,density,verbose):
        fail = 0
    ...
    def full_bootstrap_helper(self,stat,samples,dClass,sz,sigma,density,verbose):
        fail = 0
        for i,(r,d,s) in enumerate(zip(samples,dClass,sz)):
            self.data = r
            self.dist = d
    ...
    def bootstrap_CI(self, nSamples, alpha, sigma, density, verbose=False):
        ...
        stat,fail = self.bootstrap_helper(stat,samples,dClass,sz,sigma,density,verbose)
        ...
        self.data = org_data
        self.dist = org_dc
        return [stat[int((alpha/2.0)*nSamples)],org_nb,stat[int((1-alpha/2.0)*nSamples)],stat]



    def landscape_plot(self,res=0.1,sigLow=0.1,sigUp=4.0,denLow=0.1,denUp=6.0,fileName=None):
    	#run after max_likelihood
    	sig = np.arange(sigLow,sigUp,res)
    	den = np.arange(denLow,denUp,res)
    	X,Y = np.meshgrid(sig,den)
    	Z = np.array([[-log(self.update(np.array([i,j]))) for i in sig] for j in den])
    ...
    def max_likelihood(self, startSig, startDen, max_iter=10000, tol=0.00001, verbose=False):
    ...
    def full_gen_data(self, fbar, nSamples, sigma, density):
        ...
        for i,d in enumerate(self.dist2):
            if d==0:
                ...
                p = sy.nsum(lambda t: exp(self.mu2*t)*exp(-d/(4.0*ss*t))/(2.0*t), [1,sy.inf],error=False,verbose=False,method='euler-maclaurin',steps=[1000])
                p = p/denom
            phi_bar += p*nSamples
            phi[i] = p
...
class aprxCML(CML):
    def __init__(self, mu, sigma, density, dataIn, distIn, szIn, ploidy, n_terms):
        CML.__init__(self, mu, sigma, density, dataIn, distIn, szIn, ploidy, True)
        self.split = self.ndc
        self.n_t = n_terms
...
class fullCML(CML):
    def __init__(self, mu, sigma, density, dataIn, distIn, szIn, ploidy):
        CML.__init__(self, mu, sigma, density, dataIn, distIn, szIn, ploidy, False)
